...
export default async function article(_req: NextApiRequest, res: NextApiResponse) {
  const { method } = _req;
  const {
    query: { pid },
  } = _req;
  ...
  try {
    switch (method) {
      ...
      case 'PUT': {
        const { author, contents, category, created_at, updated_at, password, title, view_count } = _req.body;
