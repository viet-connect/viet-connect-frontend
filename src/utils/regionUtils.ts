  ...
  if (regionArray[1] === ALL_INCLUDED) {
    if (regionArray[0] === ALL_INCLUDED) {
      return keyword.length > 0
        ? tableContent.filter(
            (el) => el.title.includes(keyword) || el.contents.includes(keyword) || el.contact_name.includes(keyword),
          )
        : tableContent;
    }
    ...
    for (let i = 0; i < tableContent.length; i += 1) {
      ...
      if (province === targetProvince) {
        if (keyword.length > 0) {
          ...
          if (title.includes(keyword) || contents.includes(keyword) || contact_name.includes(keyword)) {
